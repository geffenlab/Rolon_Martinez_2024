...
@app.cell
def _(Path, glob, loadmat, np, os, pd, re):
    def _parse_session_and_mouse(npz_path: str):
        """
        From names like 'PSTH for S0154_Session1 TuningCurve_..._stimInfo.npz'
        return: session_label='S0154_Session1', mouse='S0154', session_only='Session1'
        """
        name = Path(npz_path).name
        m = re.search(r"PSTH for ([^ ]+)\s", name) or re.search(
            r"(S\d{3,5}_Session\d+)", name
        )
        session_label = m.group(1)
        mouse, session_only = session_label.split("_", 1)
        return session_label, mouse, session_only
    ...
    def load_and_concatenate_npz(directory, stim):
        ...
        fullpsth = np.concatenate(
            [d["allPSTH"] for d in all_data if "allPSTH" in d.files], axis=2
        )
        fullpsthS = np.concatenate(
            [d["allPSTH_S"] for d in all_data if "allPSTH_S" in d.files], axis=2
        )
        ...
        for idx, d in enumerate(all_data):
            session_id = os.path.basename(files[idx]).split(" ")[
                2
            ]  # use filename to parse

            if "clustDepth" in d.files and d["clustDepth"].size > 0:
                clust_depth_data = [
                    (depth, session_id) for depth in d["clustDepth"].tolist()
                ]
            ...
            if "rasters" in d.files and d["rasters"].size > 0:
                rasters.extend(d["rasters"].tolist())

            if "trials" in d.files and d["trials"].size > 0:
                trials.extend(d["trials"].tolist())

            if "spikeSortI" in d.files and d["spikeSortI"].size > 0:
                spikesortind.extend(d["spikeSortI"].tolist())
        ...
        for f, d in zip(files, all_data):
            session_label, mouse, session_only = _parse_session_and_mouse(f)

            # Find cluster_info.tsv
            p = Path(f).resolve()
            while p.name != "Data_Paper":
                p = p.parent
            ci_path = p / mouse / session_only / "data" / "cluster_info.tsv"

            n_units = d["allPSTH"].shape[2]  # number of PSTH units for this NPZ

            ci = pd.read_csv(ci_path, sep="\t")
            label_col = "group"  # fixed (no fallback)
            labels = ci[label_col].astype(str).str.strip().str.lower()

            # keep only good/mua, then take exactly first n_units
            labels_keep = labels[labels.isin(["good", "mua"])].tolist()
            file_labels = labels_keep[:n_units]
            labels_all.extend(file_labels)

            # also add mouse name
            mice_all.extend([mouse] * len(file_labels))

        # expose global quality_vec
        global quality_vec, mouse_vec
        quality_vec = np.array(labels_all)
        mouse_vec = np.array(mice_all)
    ...
    return load_experiment_data, quality_vec
...
def _(
    ...
    for i in range(n_neurons):
        fr_sp = sortedFullPSTH[:, spontInd_new, i].mean(axis=1)
        fr_ev = sortedFullPSTH[:, early_evoked_Ind, i].mean(axis=1)
...
@app.cell
def _(cellType, np, os, plt):
    def plot_quality_pies(
        quality_vec_masked,
        facilitation_Ind,
        suppression_Ind,
        toneIndex,
        ...
        """
        Two pie charts (MUA, GOOD) with three slices:
          - Facilitated (facilitation_Ind)
          - Suppressed (suppression_Ind)
          - Non-Significant (toneIndex)
        Shows percentages only. Colors map 1:1 to labels.
        """
        _labels = ("Facilitated", "Suppressed", "Non-Significant")
        _colors = ["#002c94", "#85000c", "#787586"]
        mua_idx = np.where(quality_vec_masked == "mua")[0]
        good_idx = np.where(quality_vec_masked == "good")[0]

        def counts_for(q_idx):
            return [
                np.intersect1d(facilitation_Ind, q_idx).size,
                np.intersect1d(suppression_Ind, q_idx).size,
                np.intersect1d(toneIndex, q_idx).size,
            ]

        mua_counts = counts_for(mua_idx)
        good_counts = counts_for(good_idx)
        pct_only = lambda pct: f"{pct:.0f}%"
        _fig, axes = plt.subplots(1, 2, figsize=(4, 2), constrained_layout=True)
        for _ax, counts, _title in [
            (axes[0], mua_counts, f"MUA (N={sum(mua_counts)})"),
            (axes[1], good_counts, f"Good units (N={sum(good_counts)})"),
        ]:
            _ax.pie(
                counts,
                labels=_labels,
        ...
        if save:
            if cellType == "PV":
                fname = "PV_celltypes_piechart.pdf"
            else:
                fname = "SST_celltypes_piechart.pdf"
            _fig.savefig(os.path.join(outdir, fname), bbox_inches="tight", dpi=dpi)
    return (plot_quality_pies,)


@app.cell
def _(np, quality_vec, sigtoneindex, toneIndex):
    # Also sort the MUA/good units by the same indices
    combined_idx = np.concatenate([sigtoneindex, toneIndex])

    quality_vec_masked = quality_vec[combined_idx]
    print(quality_vec_masked.shape)
    return (quality_vec_masked,)
...
def _(
    ...
    plot_quality_pies(
        quality_vec_masked, facilitated_Ind, suppressed_Ind, toneIndex
    )
    return
...
def _(
    ...
    def _():
        ...
        bins = np.arange(-1.5, 5.0 + 0.071, 0.071)
        # Facilitated hist
        ax.hist(
            np.clip(diff_resp_norm_EarlyOn_array[facilitated_Ind], -1.5, 5.0),
            bins=bins,
            color="#002c94",
            edgecolor="#002c94",
        )
...
@app.cell
def _(BF, cellType, depths_numeric, fig_dir_time, new_black, np, os, plt):
    def _():
        fig, ax = plt.subplots()
        fig.set_size_inches(2, 1.5)
        ax.scatter(BF, depths_numeric, c=new_black, s=0.25)
